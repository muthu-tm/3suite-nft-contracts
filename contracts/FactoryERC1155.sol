...
contract FactoryERC1155 is Ownable{
    ...
    */
    function deployERC1155(string memory _contractName, string memory _uri, uint[] memory _ids, string[] memory _names) public onlyOwner returns (address) {
    ...
    */
    function mintERC1155(uint _index, uint id, uint256 amount) public onlyOwner {
    ...
    */
    ...
    function getCountERC1155byName(uint256 _index, uint id) public view returns (uint amount) {
    // function getCountERC1155byName(uint256 _index, string calldata _name) public view returns (uint amount) {
        ...
        ERC1155Token token = tokens[_index];
        return tokens[_index].balanceOf(token.owner(), id);
    }
